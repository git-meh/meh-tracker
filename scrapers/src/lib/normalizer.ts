...
export type JobSourceType =
  "manual" | "approved_feed" | "employer_site" | "ats";
...
export function detectWorkMode(
  ...
  if (combined.includes("fully remote") || combined.includes("100% remote"))
    return "remote";
  if (combined.includes("remote first") || combined.includes("remote-first"))
    return "remote";
 ...
 */
export function parseSalaryRange(raw: string | null | undefined): {
  salaryMin: number | null;
  salaryMax: number | null;
  currency: string;
} {
